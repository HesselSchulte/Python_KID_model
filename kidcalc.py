...
def kbTeff(n_qp, SC):
   ...
   quasiparticle density."""
    Ddata = SC.Ddata
    if Ddata is not None:
        Ddata = np.array(Ddata,dtype='float64')
        kbTspl = interpolate.splrep(Ddata[2, :], Ddata[0, :])
        return interpolate.splev(n_qp, kbTspl)
    else:

        def minfunc(kbT, n_qp, SC):
            Dt = D(kbT, SC)
            return np.abs(nqp(kbT, Dt, SC) - n_qp)
...
def S21_xa(Qi, Qc, hwread, dhw, hwres, xa):
    ...
    Q = Qi * Qc / (Qi + Qc)
    dhw += hwread - hwres
    return (Q / Qi + 2j * Q * (dhw / hwres + xa)) / (1 + 2j * Q * dhw / hwres)
